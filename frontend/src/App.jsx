...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F8F7] flex flex-col font-sans text-slate-800 relative overflow-hidden">
        ...
        <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2.5 pointer-events-auto">
              ...
              <span className="text-lg font-bold text-slate-800 tracking-tight">BharatBot</span>
              <span className="text-[10px] font-bold bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full uppercase tracking-wider">Beta</span>
            </div>
            ...
            <div className="pointer-events-auto flex items-center space-x-2 glass px-3 py-1.5 rounded-full border border-slate-200/60 shadow-sm">
              <div className="h-6 w-6 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center">
                <span className="text-[11px] font-bold text-white">{user[0].toUpperCase()}</span>
              </div>
              <span className="text-sm font-medium text-slate-600 hidden sm:block">{user}</span>
      ...
      <main className="flex-1 w-full h-screen flex flex-col relative z-10">
        {!user ? (
