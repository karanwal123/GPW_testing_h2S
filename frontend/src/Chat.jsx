...
import { Send, Loader2, Globe, Sparkles, ArrowRight, Vote, Scale, Landmark, Search, BookOpen, Mic, ChevronRight } from 'lucide-react';
...
const SUGGESTIONS = [
  ...
  { icon: Landmark, text: "What are the functions of the Election Commission?", tag: "ECI"    },
...
export default function Chat({ username = "User" }) {
  ...
  const [sessionId] = useState(() => `${username}_${Date.now()}_${Math.random().toString(36).slice(2)}`);
  ...
  const send = async (override) => {
    ...
    setIsLoading(true);
    try {
      const { data } = await axios.post('/api/chat', { message: text, session_id: sessionId });
      setMessages(p => [...p, { id: Date.now() + 1, role: 'bot', text: data.reply || 'No response.' }]);
    } catch {
      setMessages(p => [...p, { id: Date.now() + 1, role: 'bot', text: "Couldn't reach the server. Please try again." }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!messages.length) {
    return (
      <div style={{
        ...
        justifyContent: 'center', padding: '24px 20px 32px',
        minHeight: '100vh', background: '#F5F4F0',
      }}>
        <div style={{ width: '100%', maxWidth: 640, display: 'flex', flexDirection: 'column', gap: 0 }}>

          {/* ── Hero ─────────────────────────────── */}
          <div className="bb-fade-up" style={{ textAlign: 'center', marginBottom: 36 }}>
            {/* Logo lockup */}
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 26, boxShadow: '0 4px 16px rgba(123,92,250,0.12)',
              }}>🇮🇳</div>
              <div style={{ textAlign: 'left' }}>
                <div className="bb-display" style={{ fontSize: 18, fontWeight: 700, color: '#0D1B2A', letterSpacing: '-0.3px' }}>
                  BharatBot
                </div>
                <div style={{ fontSize: 12, color: '#9B9690', fontWeight: 500, letterSpacing: '0.4px', textTransform: 'uppercase' }}>
                  Election Assistant
                </div>
              </div>
            </div>
            ...
            <h1 className="bb-display" style={{
              ...
              color: '#0D1B2A', lineHeight: 1.15, letterSpacing: '-1px',
              margin: 0, marginBottom: 14,
            }}>
              Your questions about<br />
              <span className="bb-brand-shimmer">Indian elections,</span> answered.
            </h1>
            <p style={{ fontSize: 15.5, color: '#7A7470', lineHeight: 1.65, margin: 0, maxWidth: 420, marginInline: 'auto' }}>
              Ask anything about voting, candidates, laws, or election results — in plain language.
            </p>
          </div>

          {/* ── Input Card ──────────────────────── */}
          <div className="bb-fade-up bb-s2 bb-input-container" style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px 12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <button
                  ...
                  style={{
                    ...
                    color: '#A9A4A0', transition: 'background 0.15s, color 0.15s',
                  }}
                  onMouseEnter={e => { e.currentTarget.style.background='#F2F0EC'; e.currentTarget.style.color='#5A5550'; }}
                  onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='#A9A4A0'; }}
                >
                  <Mic size={16} />
                </button>
                <button
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#B0AAA4', fontWeight: 500 }}>
                  <Sparkles size={12} />
                  Gemini 2.5
                </div>
                <button
          ...
          {/* ── Category Pills ───────────────────── */}
          <div className="bb-fade-up bb-s3" style={{ display: 'flex', gap: 6, overflowX: 'auto', marginBottom: 16, paddingBottom: 2 }}>
            {PILLS.map((p, i) => (
              <button
                key={i}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {SUGGESTIONS.map((s, i) => (
              ...
              >
                <div className="bb-suggestion-icon" style={{
                ...
                <span style={{ flex: 1, fontSize: 14.5, color: '#3D3A36', fontWeight: 400, lineHeight: 1.4 }}>
                  {s.text}
                </span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <span style={{
                    ...
                    background: '#F2F0EC', color: '#908B86', textTransform: 'uppercase',
                    letterSpacing: '0.4px',
                  }}>
                    {s.tag}
                  </span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '14px 24px', background: '#F5F4F0',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <BotAvatar size={36} />
          <div>
            <div className="bb-display" style={{ fontSize: 15, fontWeight: 700, color: '#0D1B2A', letterSpacing: '-0.2px' }}>
              BharatBot
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#74C98A', fontWeight: 500 }}>
      ...
      >
            ...
            <div style={{
              ...
              {msg.role === 'user' ? (
                <div style={{
                  ...
                  color: '#F2EFE8', fontFamily: 'Sora, sans-serif',
                  marginBottom: 20,
                }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <span style={{
                  fontSize: 11, fontWeight: 600, color: '#B0AAA4',
                  textTransform: 'uppercase', letterSpacing: '0.5px', paddingInline: 4,
                }}>
                  {msg.role === 'user' ? 'You' : 'BharatBot'}
                </span>

                <div
                  className={msg.role === 'user' ? 'bb-user-bubble' : 'bb-bot-bubble'}
                  style={{
                    padding: '12px 18px',
                    borderRadius: msg.role === 'user' ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                    fontSize: 15, lineHeight: 1.65,
                    boxShadow: msg.role === 'user' ? '0 4px 16px rgba(13,27,42,0.15)' : '0 2px 8px rgba(0,0,0,0.05)',
                  }}
                >
                  {msg.role === 'user' ? (
        ...
        {/* Typing indicator */}
        {isLoading && (
          <div className="bb-fade-up" style={{ display: 'flex', alignItems: 'flex-end', gap: 10 }}>
            <BotAvatar size={32} />
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#B0AAA4', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4, paddingLeft: 4 }}>
                BharatBot
              </div>
              <div className="bb-bot-bubble" style={{
                ...
                <span style={{ fontSize: 13, color: '#A09A95', fontWeight: 500 }}>
      ...
      <div style={{
        ...
        <div className="bb-input-container" style={{ padding: '4px 4px 4px 16px', display: 'flex', alignItems: 'flex-end', gap: 8 }}>
          <textarea
            ref={textareaRef}
            className="bb-textarea"
            value={input}
            ...
            rows={1}
            style={{ fontSize: 15, color: '#1E293B', lineHeight: 1.6, paddingBlock: '10px', minHeight: 40, maxHeight: 140 }}
          />

          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 6px 6px 0' }}>
            {/* Web toggle */}
            <button
              className={webSearch ? 'bb-search-on' : 'bb-search-off'}
              onClick={() => setWebSearch(v => !v)}
        ...
        {/* Disclaimer */}
        <p style={{ textAlign: 'center', fontSize: 11.5, color: '#B0AAA4', marginTop: 10, marginBottom: 0 }}>
          BharatBot may make mistakes. Verify important info on{' '}
          <a href="https://eci.gov.in" target="_blank" rel="noopener noreferrer"
            style={{ color: '#7B5CFA', textDecoration: 'underline', textUnderlineOffset: 2 }}>
            eci.gov.in
          </a>
        </p>
      </div>
